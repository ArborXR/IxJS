...
function fromEvent(obj, type, options) {
    if (isEventTarget(obj)) {
        const target = obj;
        return (0, fromeventpattern_1.fromEventPattern)((h) => target.addEventListener(type, h, options), (h) => target.removeEventListener(type, h, options));
    }
    else if (isNodeEventEmitter(obj)) {
        const target = obj;
        return (0, fromeventpattern_1.fromEventPattern)((h) => target.addListener(type, h), (h) => target.removeListener(type, h));
    }
    else {
